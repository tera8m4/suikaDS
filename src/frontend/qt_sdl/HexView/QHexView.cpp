...
//        int readAmount = m_isShiftJIS ? 2 : 1;
        QTextCodec* codec1 = QTextCodec::codecForName("Shift-JIS");
        QByteArray shiftJISchar;

        for (int xPosAscii = m_posAscii, i=0; ((lineIdx - firstLineIdx) * m_bytesPerLine + i) < data.size() && (i < m_bytesPerLine); i += 2, xPosAscii += m_charWidth * 2)
        {
            shiftJISchar.clear();
            shiftJISchar.push_back(data[(lineIdx - firstLineIdx) * (uint)m_bytesPerLine + i]);
            shiftJISchar.push_back(data[(lineIdx - firstLineIdx) * (uint)m_bytesPerLine + i + 1]);

            if ((shiftJISchar[0] & 0x80) > 0) {
                QString str = codec1->toUnicode(shiftJISchar);
                painter.drawText(xPosAscii, yPos, str);
            } else {
                painter.drawText(xPosAscii, yPos, ".");
            }
...
void QHexView::setIsShiftJIS(const bool bIsShiftJIS)
{
    m_isShiftJIS = bIsShiftJIS;
    viewport()->update();
}
